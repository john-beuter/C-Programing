...
#include <stdlib.h>
#include <string.h>
#include <limits.h> //Adding this to get the integer max
#include <unistd.h>
...
int main()
{
  ...
  for (i = 0; i < NUM_SCHEDULERS; i++)
  {
    ...
    printf("\n\n--- %s\n", schedulers[i].name); // This is where the value schedule valu is output.
    ...
    while (num_finished < NUM_PROCESSES)
    {
      ...
      /* Call the scheduler */
      int next_pid = schedulers[i].func(proc_copy, current_time); //Just get the next value. 
...
/* The scheduler being simulated is called each timestep.
   ...
   including those that have not yet arrived and those that are finished.     I want something that has arrived and is not finished
...
int first_come_first_served(process proc[], int t)
{
  int minimum = INT_MAX; //Resets the min each time. The last process should no longer exist
  int process = -1;

  for(int i = 0; i < NUM_PROCESSES; i++) //Iterate through each process
  {
    if(t >= proc[i].arrivaltime && proc[i].finished != 1) // The process has arrived and has not finished consider it to be ran
    {
      if(proc[i].arrivaltime < minimum)
      {
        minimum = proc[i].arrivaltime; 
        process = i;  // Says "hey, here is the next process"
      }
    }
  }
...
int shortest_remaining_time(process proc[], int t)
{
  ...
  //If the value that remains next is smaller than the max, run it.
  //Shioudl I then use the value and or logic or the remaining time or the run time??  

  int minimum_arrival = INT_MAX; //Resets the min each time. The last process should no longer exist
  int minimum_time = INT_MAX; //Resets the min each time. The last process should no longer exist
  int process = -1;
  ...
  for(int i = 0; i < NUM_PROCESSES; i++) //Iterate through each process
  {
    if(t >= proc[i].arrivaltime && proc[i].finished != 1) // The process has arrived and has not finished consider it to be ran
    {
      if(proc[i].remainingtime < minimum_time)  //Look at just the remaining time next
      {
        minimum_arrival = proc[i].arrivaltime;
        minimum_time = proc[i].remainingtime; 
        process = i;  
      }
    }
  }
...
int round_robin(process proc[], int t)
{
  ...
  for(int i = previous_Process; i < NUM_PROCESSES; i++) //Iterate through each process
  {
    if(t >= proc[i].arrivaltime && proc[i].finished != 1) // The process has arrived and has not finished consider it to be ran
    {
      process = i;
      previous_Process = i;
      break;
    }  //I need to get these processes t
  }
  return process; 
}
